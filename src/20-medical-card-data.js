/*
 * Asklipios — Medical Card Data
 *
 * Static data used by the Medical Card and Surgery Report modules.
 * Generated from the stable legacy userscript without changing IDs,
 * codes, labels, presets or clinical text.
 */

(function () {
    'use strict';

    const A = window.Asklipios;

    if (!A) {
        throw new Error(
            'Asklipios namespace is missing. Load 00-namespace.js first.'
        );
    }

    A.data = A.data || {};
    A.modules = A.modules || {};

    A.data.DIAGNOSIS_OPTIONS = {
        "Ρήξη μηνίσκου": [
        { id: 5666, code: "M23.30", name: "Άλλη βλάβη του μηνίσκου: Πολλαπλές εντοπίσεις" },
        { id: 5670, code: "M23.34", name: "Άλλη βλάβη του μηνίσκου: Στο πρόσθιο κέρας του έξω μηνίσκου" },
        { id: 5819, code: "M23.95", name: "Εσωτερική αποδιοργάνωση του γόνατος, μη καθορισμένη: Οπισθίου κέρατος του έξω μηνίσκου" },
        { id: 5669, code: "M23.33", name: "Άλλη βλάβη του μηνίσκου: Σε άλλο και μη καθορισμένο τμήμα του έσω μηνίσκου" },
        { id: 5665, code: "M23.29", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Μη καθορισμένος μηνίσκος" },
        { id: 5820, code: "M23.96", name: "Εσωτερική αποδιοργάνωση του γόνατος, μη καθορισμένη: Άλλου και μη καθορισμένου τμήματος του έξω μηνίσκου" },
        { id: 5817, code: "M23.93", name: "Εσωτερική αποδιοργάνωση του γόνατος, μη καθορισμένη: Έσω πλαγίου συνδέσμου ή άλλου και μη καθορισμένου τμήματος του έσω μηνίσκου" },
        { id: 5671, code: "M23.35", name: "Άλλη βλάβη του μηνίσκου: Στο οπίσθιο κέρας του έξω μηνίσκου" },
        { id: 5668, code: "M23.32", name: "Άλλη βλάβη του μηνίσκου: Στο οπίσθιο κέρας του έσω μηνίσκου" },
        { id: 9854, code: "S83.2", name: "Ρήξη μηνίσκου, πρόσφατη" },
        { id: 5815, code: "M23.91", name: "Εσωτερική αποδιοργάνωση του γόνατος, μη καθορισμένη: Προσθίου χιαστού συνδέσμου ή προσθίου κέρατος του έσω μηνίσκου" },
        { id: 5663, code: "M23.25", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Στο οπίσθιο κέρας του έξω μηνίσκου" },
        { id: 5664, code: "M23.26", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Σε άλλο και μη καθορισμένο τμήμα του έξω μηνίσκου" },
        { id: 5660, code: "M23.22", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Στο οπίσθιο κέρας του έσω μηνίσκου" },
        { id: 5672, code: "M23.36", name: "Άλλη βλάβη του μηνίσκου: Σε άλλο και μη καθορισμένο τμήμα του έξω μηνίσκου" },
        { id: 5673, code: "M23.39", name: "Άλλη βλάβη του μηνίσκου: Μη καθορισμένος μηνίσκος" },
        { id: 5816, code: "M23.92", name: "Εσωτερική αποδιοργάνωση του γόνατος, μη καθορισμένη: Οπισθίου χιαστού συνδέσμου ή οπισθίου κέρατος του έσω μηνίσκου" },
        { id: 5658, code: "M23.20", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Πολλαπλές εντοπίσεις" },
        { id: 5818, code: "M23.94", name: "Εσωτερική αποδιοργάνωση του γόνατος, μη καθορισμένη: Έξω πλαγίου συνδέσμου ή προσθίου κέρατος του έξω μηνίσκου" },
        { id: 5822, code: "M23.99", name: "Εσωτερική αποδιοργάνωση του γόνατος, μη καθορισμένη: Μη καθορισμένου συνδέσμου ή μη καθορισμένου μηνίσκου" },
        { id: 5667, code: "M23.31", name: "Άλλη βλάβη του μηνίσκου: Στο πρόσθιο κέρας του έσω μηνίσκου" },
        { id: 5662, code: "M23.24", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Στο πρόσθιο κέρας του έξω μηνίσκου" },
        { id: 5659, code: "M23.21", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Στο πρόσθιο κέρας του έσω μηνίσκου" },
        { id: 5661, code: "M23.23", name: "Βλάβη του μηνίσκου από παλαιότερη ρήξη ή τραυματισμό: Σε άλλο και μη καθορισμένο τμήμα του έσω μηνίσκου" }
        ],

        "Κάταγμα επιγονατίδας": [
            {
                id: 9711,
                code: "S82.0",
                name: "Κάταγμα της επιγονατίδας",
                default: true
            }
        ],

        "PRP": [
            { id: 5004, code: "M17.0", name: "Πρωτοπαθής εκφυλιστική αρθρίτιδα του γόνατος, αμφοτερόπλευρη" },
            { id: 5005, code: "M17.1", name: "Άλλη πρωτοπαθής εκφυλιστική αρθρίτιδα του γόνατος" },
            { id: 5006, code: "M17.2", name: "Μετατραυματική εκφυλιστική αρθρίτιδα του γόνατος, αμφοτερόπλευρη" },
            { id: 5007, code: "M17.3", name: "Άλλη μετατραυματική εκφυλιστική αρθρίτιδα του γόνατος" },
            { id: 5008, code: "M17.4", name: "Άλλη δευτεροπαθής εκφυλιστική αρθρίτιδα του γόνατος, αμφοτερόπλευρη" },
            { id: 5009, code: "M17.5", name: "Άλλη δευτεροπαθής εκφυλιστική αρθρίτιδα του γόνατος" },
            { id: 5010, code: "M17.9", name: "Εκφυλιστική αρθρίτιδα του γόνατος, μη καθορισμένη", default: true }
        ],
        "Τενοντοελυτρίτιδα De Quervain": [
            {
                id: 4834,
                code: "M65.4",
                name: "Στενωτική τενοντοελυτρίτιδα [de Quervain]",
                default: true
            }
        ],

        "Ρήξη ΠΧΣ": [

            {
                id: 9864,
                code: "S83.53",
                name: "Διάστρεμμα και διάταση της άρθρωσης του γόνατος: Ρήξη του προσθίου χιαστου συνδέσμου",
                default: true
            },
            {
                id: 5683,
                code: "M23.61",
                name: "Άλλη αυτόματη ρήξη του(των) συνδέσμου(ων) του γόνατος: Προσθίου χιαστού συνδέσμου"
            },
            {
                id: 5808,
                code: "M23.81",
                name: "Άλλη εσωτερική αποδιοργάνωση του γόνατος: Προσθίου χιαστού συνδέσμου"
            },
            {
                id: 5676,
                code: "M23.51",
                name: "Χρόνια αστάθεια του γόνατος: Προσθίου χιαστού συνδέσμου"
            }
        ],
        "Ολική αρθροπλαστική γόνατος": [
            { id: 5004, code: "M17.0", name: "Πρωτοπαθής εκφυλιστική αρθρίτιδα του γόνατος, αμφοτερόπλευρη" },
            { id: 5005, code: "M17.1", name: "Άλλη πρωτοπαθής εκφυλιστική αρθρίτιδα του γόνατος" },
            { id: 5006, code: "M17.2", name: "Μετατραυματική εκφυλιστική αρθρίτιδα του γόνατος, αμφοτερόπλευρη" },
            { id: 5007, code: "M17.3", name: "Άλλη μετατραυματική εκφυλιστική αρθρίτιδα του γόνατος" },
            { id: 5008, code: "M17.4", name: "Άλλη δευτεροπαθής εκφυλιστική αρθρίτιδα του γόνατος, αμφοτερόπλευρη" },
            { id: 5009, code: "M17.5", name: "Άλλη δευτεροπαθής εκφυλιστική αρθρίτιδα του γόνατος" },
            { id: 5010, code: "M17.9", name: "Εκφυλιστική αρθρίτιδα του γόνατος, μη καθορισμένη", default: true }
        ],

        "Ολική αρθροπλαστική ισχίου": [
            { id: 4995, code: "M16.0", name: "Πρωτοπαθής εκφυλιστική αρθρίτιδα του ισχίου, αμφοτερόπλευρη" },
            { id: 4996, code: "M16.1", name: "Άλλη πρωτοπαθής εκφυλιστική αρθρίτιδα του ισχίου" },
            { id: 4997, code: "M16.2", name: "Εκφυλιστική αρθρίτιδα του ισχίου λόγω δυσπλαστικών ισχίων, αμφοτερόπλευρη" },
            { id: 4998, code: "M16.3", name: "Άλλη δυσπλαστική εκφυλιστική αρθρίτιδα του ισχίου" },
            { id: 4999, code: "M16.4", name: "Μετατραυματική εκφυλιστική αρθρίτιδα του ισχίου, αμφοτερόπλευρη" },
            { id: 5000, code: "M16.5", name: "Άλλη μετατραυματική εκφυλιστική αρθρίτιδα του ισχίου" },
            { id: 5001, code: "M16.6", name: "Άλλη δευτεροπαθής εκφυλιστική αρθρίτιδα του ισχίου, αμφοτερόπλευρη" },
            { id: 5002, code: "M16.7", name: "Άλλη δευτεροπαθής εκφυλιστική αρθρίτιδα του ισχίου" },
            { id: 5003, code: "M16.9", name: "Εκφυλιστική αρθρίτιδα του ισχίου, μη καθορισμένη", default: true }
        ],

        "Διατροχαντήριο κάταγμα": [
            {
                id: 9522,
                code: "S72.00",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μη καθορισμένο τμήμα"
            },
            {
                id: 9523,
                code: "S72.01",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Ενδοθυλακικό"
            },
            {
                id: 9524,
                code: "S72.02",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: (Εγγύς) επίφυσης, επιφυσιόλυση"
            },
            {
                id: 9525,
                code: "S72.03",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Υποκεφαλικό"
            },
            {
                id: 9526,
                code: "S72.04",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μεσοαυχενικό"
            },
            {
                id: 9527,
                code: "S72.05",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Βασεοαυχενικό"
            },
            {
                id: 9529,
                code: "S72.10",
                name: "Κάταγμα μηριαίου οστού: Τροχαντήρα, μη καθορισμένο"
            },
            {
                id: 9530,
                code: "S72.11",
                name: "Κάταγμα μηριαίου οστού: Μεσοτροχαντήριο",
                default: true
            },
            {
                id: 9531,
                code: "S72.2",
                name: "Υποτροχαντήριο κάταγμα"
            },
            {
                id: 9532,
                code: "S72.3",
                name: "Κάταγμα της διάφυσης του μηριαίου οστού"
            },
            {
                id: 9540,
                code: "S72.9",
                name: "Κάταγμα του μηριαίου οστού, τμήμα μη καθορισμένο"
            }
        ],

        "Κάταγμα αυχένα μηριαίου": [
            {
                id: 9522,
                code: "S72.00",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μη καθορισμένο τμήμα"
            },
            {
                id: 9523,
                code: "S72.01",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Ενδοθυλακικό"
            },
            {
                id: 9524,
                code: "S72.02",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: (Εγγύς) επίφυσης, επιφυσιόλυση"
            },
            {
                id: 9525,
                code: "S72.03",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Υποκεφαλικό",
                default: true
            },
            {
                id: 9526,
                code: "S72.04",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μεσοαυχενικό"
            },
            {
                id: 9527,
                code: "S72.05",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Βασεοαυχενικό"
            },
            {
                id: 9529,
                code: "S72.10",
                name: "Κάταγμα μηριαίου οστού: Τροχαντήρα, μη καθορισμένο"
            },
            {
                id: 9530,
                code: "S72.11",
                name: "Κάταγμα μηριαίου οστού: Μεσοτροχαντήριο"
            },
            {
                id: 9531,
                code: "S72.2",
                name: "Υποτροχαντήριο κάταγμα"
            },
            {
                id: 9532,
                code: "S72.3",
                name: "Κάταγμα της διάφυσης του μηριαίου οστού"
            },
            {
                id: 9540,
                code: "S72.9",
                name: "Κάταγμα του μηριαίου οστού, τμήμα μη καθορισμένο"
            }
        ],
        "Κάταγμα διάφυσης μηριαίου οστού": [
            {
                id: 9522,
                code: "S72.00",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μη καθορισμένο τμήμα"
            },
            {
                id: 9523,
                code: "S72.01",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Ενδοθυλακικό"
            },
            {
                id: 9524,
                code: "S72.02",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: (Εγγύς) επίφυσης, επιφυσιόλυση"
            },
            {
                id: 9525,
                code: "S72.03",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Υποκεφαλικό"
            },
            {
                id: 9526,
                code: "S72.04",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μεσοαυχενικό"
            },
            {
                id: 9527,
                code: "S72.05",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Βασεοαυχενικό"
            },
            {
                id: 9529,
                code: "S72.10",
                name: "Κάταγμα μηριαίου οστού: Τροχαντήρα, μη καθορισμένο"
            },
            {
                id: 9530,
                code: "S72.11",
                name: "Κάταγμα μηριαίου οστού: Μεσοτροχαντήριο"
            },
            {
                id: 9531,
                code: "S72.2",
                name: "Υποτροχαντήριο κάταγμα"
            },
            {
                id: 9532,
                code: "S72.3",
                name: "Κάταγμα της διάφυσης του μηριαίου οστού",
                default: true
            },
            {
                id: 9540,
                code: "S72.9",
                name: "Κάταγμα του μηριαίου οστού, τμήμα μη καθορισμένο"
            }
        ],
        "Υποτροχαντήριο κάταγμα": [
            {
                id: 9522,
                code: "S72.00",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μη καθορισμένο τμήμα"
            },
            {
                id: 9523,
                code: "S72.01",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Ενδοθυλακικό"
            },
            {
                id: 9524,
                code: "S72.02",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: (Εγγύς) επίφυσης, επιφυσιόλυση"
            },
            {
                id: 9525,
                code: "S72.03",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Υποκεφαλικό"
            },
            {
                id: 9526,
                code: "S72.04",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Μεσοαυχενικό"
            },
            {
                id: 9527,
                code: "S72.05",
                name: "Κάταγμα του αυχένα του μηριαίου οστού: Βασεοαυχενικό"
            },
            {
                id: 9529,
                code: "S72.10",
                name: "Κάταγμα μηριαίου οστού: Τροχαντήρα, μη καθορισμένο"
            },
            {
                id: 9530,
                code: "S72.11",
                name: "Κάταγμα μηριαίου οστού: Μεσοτροχαντήριο"

            },
            {
                id: 9531,
                code: "S72.2",
                name: "Υποτροχαντήριο κάταγμα",
                default: true
            },
            {
                id: 9532,
                code: "S72.3",
                name: "Κάταγμα της διάφυσης του μηριαίου οστού"
            },
            {
                id: 9540,
                code: "S72.9",
                name: "Κάταγμα του μηριαίου οστού, τμήμα μη καθορισμένο"
            }
        ],

        "Εκτινασσόμενος δάκτυλος": [
            {
                id: 4833,
                code: "M65.3",
                name: "Εκτινασσόμενος δάκτυλος",
                default: true
            }
        ],

        "Σύνδρομο καρπιαίου σωλήνα": [
            {
                id: 2933,
                code: "G56.0",
                name: "Σύνδρομο καρπιαίου σωλήνα",
                default: true
            }
        ],
        "Κάταγμα κνήμης": [
            { id: 9712, code: "S82.11", name: "Κάταγμα του εγγύς άκρου του οστού της κνήμης: Με κάταγμα της περόνης" },
            { id: 9713, code: "S82.18", name: "Κάταγμα του εγγύς άκρου του οστού της κνήμης: Άλλο" },
            { id: 9714, code: "S82.21", name: "Κάταγμα της διάφυσης του οστού της κνήμης: Με κάταγμα της περόνης" },
            { id: 9715, code: "S82.28", name: "Κάταγμα της διάφυσης του οστού της κνήμης: Άλλο"},
            { id: 9716, code: "S82.31", name: "Κάταγμα του άπω άκρου του οστού της κνήμης: Με κάταγμα της περόνης" },
            { id: 9717, code: "S82.38", name: "Κάταγμα του άπω άκρου του οστού της κνήμης: Άλλο" },
            { id: 9842, code: "S82.7", name: "Πολλαπλά κατάγματα της κνήμης" },
            { id: 9846, code: "S82.9", name: "Κάταγμα της κνήμης, τμήμα μη καθορισμένο", default: true  }
        ],

        "Κατάγματα σφυρών": [
            {
                id: 9843,
                code: "S82.81",
                name: "Κατάγματα άλλων τμημάτων της κνήμης: Αμφισφύριο",
                course: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση αμφισφυρίου κατάγματος.",
                therapy: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση αμφισφυρίου κατάγματος."
            },
            {
                id: 9844,
                code: "S82.82",
                name: "Κατάγματα άλλων τμημάτων της κνήμης: Τρισφύριο",
                course: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση τρισφυρίου κατάγματος.",
                therapy: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση τρισφυρίου κατάγματος."
            },
            {
                id: 9840,
                code: "S82.5",
                name: "Κάταγμα του έσω σφυρού",
                course: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος έσω σφυρού.",
                therapy: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος έσω σφυρού."
            },
            {
                id: 9841,
                code: "S82.6",
                name: "Κάταγμα του έξω σφυρού",
                default: true,
                course: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος έξω σφυρού.",
                therapy: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος έξω σφυρού."
            }
        ],
        "Κάταγμα οσφυϊκού σπονδύλου": [
            {
                id: 9493,
                code: "S32.00",
                name: "Κάταγμα οσφυϊκού σπονδύλου: Ύψος βλάβης μη καθορισμένο",
                default: true
            },
            {
                id: 9494,
                code: "S32.01",
                name: "Κάταγμα οσφυϊκού σπονδύλου: Ο1"
            },
            {
                id: 9495,
                code: "S32.02",
                name: "Κάταγμα οσφυϊκού σπονδύλου: Ο2"
            },
            {
                id: 9496,
                code: "S32.03",
                name: "Κάταγμα οσφυϊκού σπονδύλου: Ο3"
            },
            {
                id: 9497,
                code: "S32.04",
                name: "Κάταγμα οσφυϊκού σπονδύλου: Ο4"
            },
            {
                id: 9498,
                code: "S32.05",
                name: "Κάταγμα οσφυϊκού σπονδύλου: Ο5"
            }
        ],
        "Κάταγμα θωρακικού σπονδύλου": [
            {
                id: 9037,
                code: "S22.00",
                name: "Κάταγμα θωρακικού σπονδύλου: Ύψος βλάβης μη καθορισμένο",
                default: true
            },
            {
                id: 9038,
                code: "S22.01",
                name: "Κάταγμα θωρακικού σπονδύλου: Θ1 και Θ2"
            },
            {
                id: 9039,
                code: "S22.02",
                name: "Κάταγμα θωρακικού σπονδύλου: Θ3 και Θ4"
            },
            {
                id: 9040,
                code: "S22.03",
                name: "Κάταγμα θωρακικού σπονδύλου: Θ5 και Θ6"
            },
            {
                id: 9041,
                code: "S22.04",
                name: "Κάταγμα θωρακικού σπονδύλου: Θ7 και Θ8"
            },
            {
                id: 9042,
                code: "S22.05",
                name: "Κάταγμα θωρακικού σπονδύλου: Θ9 και Θ10"
            },
            {
                id: 9043,
                code: "S22.06",
                name: "Κάταγμα θωρακικού σπονδύλου: Θ11 και Θ12"
            }
        ],
        "Κάταγμα άπω άκρου κερκίδας": [
            {
                id: 10397,
                code: "S52.50",
                name: "Κάταγμα του άπω άκρου της κερκίδας: Μη καθορισμένο τμήμα",
                default: true
            },
            {
                id: 10398,
                code: "S52.51",
                name: "Κάταγμα του άπω άκρου της κερκίδας: Εκτατικού τύπου"
            },
            {
                id: 10399,
                code: "S52.52",
                name: "Κάταγμα του άπω άκρου της κερκίδας: Καμπτικού τύπου"
            },
            {
                id: 10400,
                code: "S52.59",
                name: "Κάταγμα του άπω άκρου της κερκίδας: Άλλα και πολλαπλά τμήματα"
            }
        ],
        "Ρήξη στροφικού πετάλου ώμου": [
            { id: 3689, code: "M75.1", name: "Σύνδρομο του τενόντιου πετάλου του ώμου", default: true },
            { id: 3692, code: "M75.4", name: "Σύνδρομο πρόσκρουσης του ώμου" },
            { id: 3695, code: "M75.9", name: "Βλάβη του ώμου, μη καθορισμένη" }
        ]

    };

    A.data.MEDICAL_CARD_PRESETS = {
    "Ρήξη μηνίσκου": {
        course: "Αρθροσκοπική αποκατάσταση ρήξης έσω/έξω μηνίσκου - Μερική μηνισκεκτομή - Συρραφή μηνίσκου",
        therapy: "Αρθροσκοπική αποκατάσταση ρήξης έσω/έξω μηνίσκου - Μερική μηνισκεκτομή - Συρραφή μηνίσκου"
    },

    "Κάταγμα επιγονατίδας": {
        course: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος επιγονατίδας - tension band",
        therapy: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος επιγονατίδας - tension band"
    },

    "Κάταγμα βραχιονίου": {
        course: "Μετεγχειρητική πορεία ομαλή.",
        therapy: "Χειρουργική αντιμετώπιση κατάγματος βραχιονίου."
    },

    "PRP": {
        course: "Ενδαρθρική έγχυση PRP γόνατος",
        therapy: "Ενδαρθρική έγχυση PRP γόνατος"
    },
    "Τενοντοελυτρίτιδα De Quervain": {
        course: "Διανοιξη ελύτρου για αποσυμπίεση 1ου διαμερίσματος De Quervain",
        therapy: "Διανοιξη ελύτρου για αποσυμπίεση 1ου διαμερίσματος De Quervain"
    },

    "Ρήξη ΠΧΣ": {
        course: "Αρθροσκοπική αποκατάσταση ρήξης προσθίου χιαστού συνδέσμου με αυτογενές μόσχευμα οπισθίων μηριαίων (Hamstrings) ή επιγονατιδικού τένοντα (Patellar tendon)",
        therapy: "Αρθροσκοπική αποκατάσταση ρήξης προσθίου χιαστού συνδέσμου με αυτογενές μόσχευμα οπισθίων μηριαίων (Hamstrings) ή επιγονατιδικού τένοντα (Patellar tendon)"
    },
    "Διατροχαντήριο κάταγμα": {
        course: "Ενδομυελική ήλωση διατροχαντηρίου κατάγματος",
        therapy: "Ενδομυελική ήλωση διατροχαντηρίου κατάγματος"
    },
    "Υποτροχαντήριο κάταγμα": {
        course: "Ενδομυελική ήλωση υποτροχαντηρίου κατάγματος",
        therapy: "Ενδομυελική ήλωση υποτροχαντηρίου κατάγματος"
    },
    "Ολική αρθροπλαστική γόνατος": {
        course: "Ολική αρθροπλαστική γόνατος",
        therapy: "Ολική αρθροπλαστική γόνατος"
    },
    "Εκτινασσόμενος δάκτυλος": {
        course: "Διάνοιξη Α1 pulley",
        therapy: "Διάνοιξη Α1 pulley"
    },
    "Κάταγμα αυχένα μηριαίου": {
        course: "",
        therapy: "Χειρουργική αντιμετώπιση κατάγματος αυχένα μηριαίου."
    },
    "Ολική αρθροπλαστική ισχίου": {
        course: "Ολική αρθροπλαστική ισχίου",
        therapy: "Ολική αρθροπλαστική ισχίου"
    },
    "Σύνδρομο καρπιαίου σωλήνα": {
        course: "Διάνοιξη εγκαρσίου συνδέσμου καρπού (Carpal Tunnel Release)",
        therapy: "Διάνοιξη εγκαρσίου συνδέσμου καρπού (Carpal Tunnel Release)"
    },
    "Κάταγμα διάφυσης μηριαίου οστού": {
        course: "Ενδομυελική ήλωση κατάγματος διάφυσης μηριαίου οστού",
        therapy: "Ενδομυελική ήλωση κατάγματος διάφυσης μηριαίου οστού"
    },

    "Κάταγμα κνήμης": {
        course: "",
        therapy: ""
    },

    "Κατάγματα σφυρών": {
        course: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος σφυρών.",
        therapy: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος σφυρών."
    },

    "Κάταγμα οσφυϊκού σπονδύλου": {
        course: "Νευρολογικά ακέραιος ασθενής.",
        therapy: "Συντηρητική αντιμετώπιση με κηδεμόνα και αναλγητική αγωγή."
    },

    "Κάταγμα θωρακικού σπονδύλου": {
        course: "Νευρολογικά ακέραιος ασθενής.",
        therapy: "Συντηρητική αντιμετώπιση με κηδεμόνα και αναλγητική αγωγή."
    },

    "Κάταγμα άπω άκρου κερκίδας": {
        course: "Μετεγχειρητική πορεία ομαλή.",
        therapy: "Ανοικτή ανάταξη και εσωτερική οστεοσύνθεση κατάγματος άπω άκρου κερκίδας."
    },
    "Ρήξη στροφικού πετάλου ώμου": {
        course: "",
        therapy: ""
    }
};

    A.data.MEDICAL_CARD_COMPANIES = [
        "Medinor",
        "Depuy Synthes",
        "DMPS",
        "Pharmagenesis",
        "Smith & Nephew",
        "Cooper",
        "Zimmer Biomet",
        "Άλλη"
    ];

    A.data.MEDICAL_ACT_OPTIONS = {
        "Ρήξη μηνίσκου": [
            {
                id: 14910,
                code: "381.02.00.17",
                name: "Αρθροσκοπική επέμβαση στο χόνδρο της Άρθρωσης και τους μηνίσκους: Εκτομή παθολογικών ιστών αρθρικού χόνδρου: Άρθρωση του γόνατος",
                requireLr: true
            },
            {
                id: 14693,
                code: "381.02.98.17",
                name: "Αρθροσκοπική επέμβαση στο χόνδρο της Άρθρωσης και τους μηνίσκους: Άλλη: Άρθρωση του γόνατος",
                requireLr: true
            },
            {
                id: 15462,
                code: "381.02.15.17",
                name: "Αρθροσκοπική επέμβαση στο χόνδρο της Άρθρωσης και τους μηνίσκους: Διάνοιξη του οστού υπό τον χόνδρο (π.χ. σε επέμβαση Pridie, τρυπανισμούς): Άρθρωση του γόνατος",
                requireLr: true
            },
            {
                id: 15384,
                code: "381.02.14.17",
                name: "Αρθροσκοπική επέμβαση στο χόνδρο της Άρθρωσης και τους μηνίσκους: Χονδροπλαστική: Άρθρωση του γόνατος",
                requireLr: true
            }
        ],
        "Κάταγμα επιγονατίδας": [
            {
                id: 11864,
                code: "379.04.01.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με σύρμα ή ταινία ελκυσμού: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12916,
                code: "379.03.20.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με δακτυλίους καθήλωσης: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12901,
                code: "379.03.19.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα σταθερής γωνίας: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 11786,
                code: "379.04.00.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με βίδα: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12112,
                code: "379.04.06.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική και εσωτερική οστεοσύνθεση: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12603,
                code: "379.04.19.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα σταθερής γωνίας: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12337,
                code: "379.03.06.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική οστεοσύνθεση: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12439,
                code: "379.04.15.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με απορροφήσιμο υλικό: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12994,
                code: "379.03.98.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Άλλη: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12678,
                code: "379.04.20.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με δακτυλίους καθήλωσης: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12693,
                code: "379.04.98.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Άλλη: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 11879,
                code: "379.04.02.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12172,
                code: "379.03.02.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με σύρμα ή ταινία ελκυσμού: Επιγονατίδα",
                requireLr: true,
                default: true
            },
            {
                id: 12020,
                code: "379.03.00.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Ανοικτή ανάταξη επιφυσιόλυσης χωρίς οστεοσύνθεση: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12127,
                code: "379.04.07.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12671,
                code: "379.03.15.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με απορροφήσιμο υλικό: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12204,
                code: "379.04.08.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών και ανακατασκευή της αρθρικής επιφάνειας: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12504,
                code: "379.03.09.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών και ανακατασκευή της αρθρικής επιφάνειας: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12096,
                code: "379.03.01.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με βίδα: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12352,
                code: "379.03.07.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική και εσωτερική οστεοσύνθεση: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12428,
                code: "379.03.08.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12034,
                code: "379.04.05.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική οστεοσύνθεση: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12363,
                code: "379.04.14.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με Αγκράφα [clip]: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12527,
                code: "379.04.17.18",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Χωρίς οστεοσύνθεση: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12824,
                code: "379.03.17.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Χωρίς οστεοσύνθεση: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12187,
                code: "379.03.03.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα: Επιγονατίδα",
                requireLr: true
            },
            {
                id: 12656,
                code: "379.03.14.18",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με Αγκράφα [clip]: Επιγονατίδα",
                requireLr: true
            }
        ],
        "Κατάγματα σφυρών": [
            {
                id: 12432,
                code: "379.03.08.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12100,
                code: "379.03.01.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με βίδα: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12508,
                code: "379.03.09.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών και ανακατασκευή της αρθρικής επιφάνειας: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12191,
                code: "379.03.03.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12998,
                code: "379.03.98.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Άλλη: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12176,
                code: "379.03.02.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με σύρμα ή ταινία ελκυσμού: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12417,
                code: "379.03.07.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική και εσωτερική οστεοσύνθεση: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12905,
                code: "379.03.19.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα σταθερής γωνίας: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12341,
                code: "379.03.06.24",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική οστεοσύνθεση: Άπω άκρο περόνης",
                requireLr: true
            },
            {
                id: 12430,
                code: "379.03.08.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12206,
                code: "379.04.08.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών και ανακατασκευή της αρθρικής επιφάνειας: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 11788,
                code: "379.04.00.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με βίδα: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12756,
                code: "379.04.98.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Άλλη: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 11881,
                code: "379.04.02.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12996,
                code: "379.03.98.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Άλλη: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12036,
                code: "379.04.05.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική οστεοσύνθεση: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12605,
                code: "379.04.19.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα σταθερής γωνίας: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12174,
                code: "379.03.02.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με σύρμα ή ταινία ελκυσμού: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12189,
                code: "379.03.03.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12339,
                code: "379.03.06.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική οστεοσύνθεση: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12506,
                code: "379.03.09.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών και ανακατασκευή της αρθρικής επιφάνειας: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12114,
                code: "379.04.06.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική και εσωτερική οστεοσύνθεση: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12129,
                code: "379.04.07.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12415,
                code: "379.03.07.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική και εσωτερική οστεοσύνθεση: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 12098,
                code: "379.03.01.21",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με βίδα: Άπω άκρο κνήμης",
                requireLr: true
            },
            {
                id: 11866,
                code: "379.04.01.21",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με σύρμα ή ταινία ελκυσμού: Άπω άκρο κνήμης",
                requireLr: true
            }
        ],
        "PRP": [
            {
                id: 20129,
                code: "502.00.05",
                name: "Θεραπευτική εγχύση: Άρθρωση ή αρθρικός θύλακας",
                requireLr: true,
                default: true
            }
        ],
        "Τενοντοελυτρίτιδα De Quervain": [
            {
                id: 19038,
                code: "384.00.00.08",
                name: "Επεμβάσεις των τενόντων της άκρας χείρας: Τομή: Έλυτρο τενόντων του αντίχειρα",
                requireLr: true,
                default: true
            }
        ],
        "Ρήξη ΠΧΣ": [
            {
                id: 14707,
                code: "381.03.04",
                name: "Αρθροσκοπική επανασταθεροποίηση και πλαστική αποκατάσταση της άρθρωσης του γόνατος: Συνδεσμοπλαστική προσθίου χιαστού συνδέσμου με άλλο αυτογενή τένοντα",
                requireLr: true,
                default: true
            },
            {
                id: 14706,
                code: "381.03.03",
                name: "Αρθροσκοπική επανασταθεροποίηση και πλαστική αποκατάσταση της άρθρωσης του γόνατος: Συνδεσμοπλαστική προσθίου χιαστού συνδέσμου με αυτογενή επιγονατιδικό τένοντα",
                requireLr: true
            },
            {
                id: 14775,
                code: "381.03.16",
                name: "Αρθροσκοπική επανασταθεροποίηση και πλαστική αποκατάσταση της άρθρωσης του γόνατος: Συνδεσμοπλαστική προσθίου χιαστού συνδέσμου με αλλογενές τενόντιο μόσχευμα",
                requireLr: true
            }
        ],
        "Ρήξη στροφικού πετάλου ώμου": [
            {
                id: 14843,
                code: "381.04.04",
                name: "Αρθροσκοπική επανασταθεροποίηση και πλαστική αποκατάσταση της άρθρωσης του ώμου: Άλλη αποκατάσταση του στροφικού πετάλου",
                requireLr: true,
                default: true
            },
            {
                id: 16852,
                code: "380.05.07",
                name: "Ανοικτή χειρουργική σταθεροποίηση και πλαστική αποκατάσταση των θυλακοσυνδεσμικών στοιχείων της άρθρωσης του ώμου: Αποκατάσταση του πετάλου στροφέων με συρραφή",
                requireLr: true
            }
        ],
        "Κάταγμα άπω άκρου κερκίδας": [
            {
                id: 11873,
                code: "379.04.02.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα: Άπω άκρο κερκίδας",
                requireLr: true,
                default: true
            },
            {
                id: 13004,
                code: "379.04.00.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με βίδα: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12043,
                code: "379.04.06.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική και εσωτερική οστεοσύνθεση: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12269,
                code: "379.03.06.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική οστεοσύνθεση: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12988,
                code: "379.03.98.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Άλλη: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12346,
                code: "379.03.07.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική και εσωτερική οστεοσύνθεση: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12498,
                code: "379.03.09.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών και ανακατασκευή της αρθρικής επιφάνειας: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 11966,
                code: "379.04.05.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με εξωτερική οστεοσύνθεση: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12597,
                code: "379.04.19.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα σταθερής γωνίας: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12181,
                code: "379.03.03.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12422,
                code: "379.03.08.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12833,
                code: "379.03.19.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με πλάκα σταθερής γωνίας: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12014,
                code: "379.03.00.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Ανοικτή ανάταξη επιφυσιόλυσης χωρίς οστεοσύνθεση: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12198,
                code: "379.04.08.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών και ανακατασκευή της αρθρικής επιφάνειας: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12121,
                code: "379.04.07.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με συνδυασμό υλικών: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12090,
                code: "379.03.01.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με βίδα: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12687,
                code: "379.04.98.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Άλλη: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12521,
                code: "379.04.17.06",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Χωρίς οστεοσύνθεση: Άπω άκρο κερκίδας",
                requireLr: true
            },
            {
                id: 12755,
                code: "379.03.17.06",
                name: "Ανοικτή ανάταξη απλού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Χωρίς οστεοσύνθεση: Άπω άκρο κερκίδας",
                requireLr: true
            }
        ],
        "Σύνδρομο καρπιαίου σωλήνα": [
            {
                id: 18476,
                code: "384.01.01.01",
                name: "Επεμβάσεις στους συνδέσμους της άκρας χείρας: Διατομή: Εγκάρσιος σύνδεσμος",
                requireLr: true,
                default: true
            }
        ],

        "Εκτινασσόμενος δάκτυλος": [
            {
                id: 19038,
                code: "384.00.00.08",
                name: "Επεμβάσεις των τενόντων της άκρας χείρας: Τομή: Έλυτρο τενόντων του αντίχειρα",
                requireLr: true
            },
            {
                id: 19037,
                code: "384.00.00.07",
                name: "Επεμβάσεις των τενόντων της άκρας χείρας: Τομή: Έλυτρο τενόντων των δακτύλων",
                requireLr: true,
                default: true
            }
        ],
        "Διατροχαντήριο κάταγμα": [
            {
                id: 12287,
                code: "379.04.11.15",
                name: "Ανοικτή ανάταξη συντριπτικού κατάγματος στην αρθρική επιφάνεια μακρού οστού: Με ενδομυελικό ήλο: Εγγύς άκρο μηριαίου",
                requireLr: true,
                default: true
            }
        ],

        "Κάταγμα αυχένα μηριαίου": [
            {
                id: 15168,
                code: "382.00.04.01",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του ισχίου: Διπολική πρόθεση: Με τσιμέντο",
                requireLr: true,
                default: true
            },
            {
                id: 15099,
                code: "382.00.00.00",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του ισχίου: Ολική ενδοπρόθεση: Χωρίς τσιμέντο",
                requireLr: true
            },
            {
                id: 15100,
                code: "382.00.00.01",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του ισχίου: Ολική ενδοπρόθεση: Με τσιμέντο",
                requireLr: true
            }

        ],

        "Ολική αρθροπλαστική ισχίου": [
            {
                id: 15099,
                code: "382.00.00.00",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του ισχίου: Ολική ενδοπρόθεση: Χωρίς τσιμέντο",
                requireLr: true,
                default: true
            },
            {
                id: 15100,
                code: "382.00.00.01",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του ισχίου: Ολική ενδοπρόθεση: Με τσιμέντο",
                requireLr: true
            },
            {
                id: 15101,
                code: "382.00.00.02",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του ισχίου: Ολική ενδοπρόθεση: Υβρίδιο (τσιμέντο στο ένα τμήμα)",
                requireLr: true
            }
        ],
        "Ολική αρθροπλαστική γόνατος": [
            {
                id: 15547,
                code: "382.02.01.00",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, ασύνδετη, χωρίς αντικατάσταση επιγονατίδας: Χωρίς τσιμέντο",
                requireLr: true
            },
            {
                id: 15548,
                code: "382.02.01.01",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, ασύνδετη, χωρίς αντικατάσταση επιγονατίδας: Με τσιμέντο",
                requireLr: true,
                default: true
            },
            {
                id: 15549,
                code: "382.02.01.02",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, ασύνδετη, χωρίς αντικατάσταση επιγονατίδας: Υβρίδιο (τσιμέντο στο ένα τμήμα)",
                requireLr: true
            },
            {
                id: 15553,
                code: "382.02.03.00",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, μερικώς συνδεδεμένη, χωρίς αντικατάσταση επιγονατίδας: Χωρίς τσιμέντο",
                requireLr: true
            },
            {
                id: 15554,
                code: "382.02.03.01",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, μερικώς συνδεδεμένη, χωρίς αντικατάσταση επιγονατίδας: Με τσιμέντο",
                requireLr: true
            },
            {
                id: 15555,
                code: "382.02.03.02",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, μερικώς συνδεδεμένη, χωρίς αντικατάσταση επιγονατίδας: Υβρίδιο (τσιμέντο στο ένα τμήμα)",
                requireLr: true
            },
            {
                id: 15559,
                code: "382.02.06.00",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, συνδεδεμένη, χωρίς αντικατάσταση επιγονατίδας: Χωρίς τσιμέντο",
                requireLr: true
            },
            {
                id: 15560,
                code: "382.02.06.01",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, συνδεδεμένη, χωρίς αντικατάσταση επιγονατίδας: Με τσιμέντο",
                requireLr: true
            },
            {
                id: 15561,
                code: "382.02.06.02",
                name: "Εμφύτευση ενδοπρόθεσης στην άρθρωση του γόνατος: Πρόθεση ολικής αρθροπλαστικής επιφανείας, συνδεδεμένη, χωρίς αντικατάσταση επιγονατίδας: Υβρίδιο (τσιμέντο στο ένα τμήμα)",
                requireLr: true
            }
        ]
    };

    A.data.DOCTOR_FOLLOWUP_TEXTS = {
        "ΜΠΑΡΓΙΩΤΑΣ ΚΩΝ/ΝΟΣ": {
            time: "09:00-11:00",
            doctorLabel: "κ. Μπαργιώτας"
        },
        "ΣΑΙΝΗΣ ΙΩΑΝΝΗΣ": {
            time: "10:00-11:00",
            doctorLabel: "κ. Σαϊνης"
        },
        "ΧΡΙΣΤΟΔΟΥΛΟΥ ΣΩΤΗΡΙΟΣ": {
            time: "10:00-11:00",
            doctorLabel: "κ. Χριστοδούλου"
        },
        "ΜΠΑΚΑΓΙΑΝΝΗΣ ΓΕΩΡΓΙΟΣ": {
            time: "08:30-12:00",
            doctorLabel: "κ. Μπακαγιάννης"
        },
        "ΚΟΛΟΒΟΣ ΣΤΥΛΙΑΝΟΣ": {
            time: "09:00-10:00",
            doctorLabel: "κ. Κολοβός"
        },
        "ΑΝΑΣΤΑΣΟΠΟΥΛΟΣ ΗΛΙΑΣ": {
            time: "09:00-10:00",
            doctorLabel: "κ. Αναστασόπουλος"
        }
    };

    A.data.ANTICOAGULATION_TEXTS = {
        "Xarelto10": `Αντιπηκτική αγωγή TB XARELTO 10 mg 1X1 για 10 ημέρες.`,
        "Xarelto20": `Αντιπηκτική αγωγή TB XARELTO 20 mg 1X1.`,
        "Eliquis5": `Αντιπηκτική αγωγή ΤΒ ELIQUIS 5mg ως ελάμβανε.`,
        "Eliquis2.5": `Αντιπηκτική αγωγή ΤΒ ELIQUIS 2.5mg ως ελάμβανε.`,
        "Pradaxa": `Συνέχιση της αγωγής με Pradaxa ως ελάμβανε.`,
        "Sintrom": `Συνέχιση της αγωγής με Sintrom με τακτικό έλεγχο INR.`,
        "Ivor3500": `Αντιπηκτική αγωγή IVORINJ.SOL 3500IUAnti-Xa/0,2Msyr 1x1 για 1 μήνα.`,
        "Ivor2500": `Αντιπηκτική αγωγή IVORINJ.SOL 2500IUAnti-Xa/0,2Msyr 1x1 για 1 μήνα.`,
        "Clexane40": `Αντιπηκτική αγωγή CLEXANE INJ.SOL 4000ANTI-XA IU/0,4ML 1x1 για 1 μήνα.`,
        "Clexane60": `Αντιπηκτική αγωγή CLEXANE INJ.SOL 6000ANTI-XA IU/0,4ML 1x1 για 1 μήνα.`,
        "Arixtra2.5": `Αντιπηκτική αγωγή ARIXTRA INJ.SOL 2,5MG/0,5ML PF,SYR 1x1 για 1 μήνα.`
    };

    A.data.DEFAULT_DISCHARGE_TEXT = `Επανεξέταση στα εξωτερικά ιατρέια Ορθοπαιδικής Κλινικής`;

    A.data.DOCTOR_DISCHARGE_TEXTS = {
        "ΜΠΑΡΓΙΩΤΑΣ ΚΩΝ/ΝΟΣ": `Επανεξέταση στα Τακτικά Εξωτερικά Ιατρεία της Ορθοπαιδικής Κλινικής την Πέμπτη 9:00-11:00 (κ. Μπαργιώτας)`,
        "ΣΑΙΝΗΣ ΙΩΑΝΝΗΣ": `Επανεξέταση στα Τακτικά Εξωτερικά Ιατρεία της Ορθοπαιδικής Κλινικής τη Δευτέρα το πρωί 10:00-11:00 (κύριος Σαϊνης)`,
        "ΧΡΙΣΤΟΔΟΥΛΟΥ ΣΩΤΗΡΙΟΣ": `Επανεξέταση στα Τακτικά Εξωτερικά Ιατρεία της Ορθοπαιδικής Κλινικής τη Δευτέρα το πρωί 10:00-11:00 (κύριος Χριστοδούλου)`,
        "ΜΠΑΚΑΓΙΑΝΝΗΣ ΓΕΩΡΓΙΟΣ": `Επανεξέταση στα Τακτικά Εξωτερικά Ιατρεία της Ορθοπαιδικής Κλινικής την Τρίτη το πρωί 8:30-12:00 (κύριος Μπακαγιάννης)`,
        "ΚΟΛΟΒΟΣ ΣΤΥΛΙΑΝΟΣ": `Επανεξέταση στα Τακτικά Εξωτερικά Ιατρεία της Ορθοπαιδικής Κλινικής την Παρασκευή το πρωί 9:00-10:00 (κύριος Κολοβός)`,
        "ΑΝΑΣΤΑΣΟΠΟΥΛΟΣ ΗΛΙΑΣ": `Επανεξέταση στα Τακτικά Εξωτερικά Ιατρεία της Ορθοπαιδικής Κλινικής την Παρασκευή το πρωί 9:00-10:00 (κύριος Αναστασόπουλος)`
    };

    A.data.PRESET_DISCHARGE_TEXTS = {
        "Ολική αρθροπλαστική γόνατος": `Βάδιση με πλήρη φόρτιση του χειρουργημένου σκέλους με βακτηρία τύπου ''π''.`,
        "Ολική αρθροπλαστική ισχίου": `Οδηγίες για ΟΑΙ.`,
        "Κάταγμα άπω άκρου κερκίδας": `Οδηγίες για κάταγμα άπω άκρου κερκίδας.`
    };

    A.data.SURGERY_DESCRIPTIONS = {
        "Κάταγμα επιγονατίδας": `Με τον ασθενή σε ύπτια θέση, υπό ραχιαία αναισθησία και ισχαιμική περίδεση, έγινε άσηπτη προετοιμασία του δέρματος και του χειρουργικού πεδίου. Πραγματοποιήθηκε τομή επί της πρόσθιας επιφάνειας του γόνατος. Έγινε ανάταξη του κατάγματος και σταθεροποίησή του με 2 K-wires και σύρμα κατά την τεχνική tension band. Η ανάταξη και η θέση των υλικών επιβεβαιώθηκαν με ακτινοσκοπικό έλεγχο. Ακολούθησε επιμελής έκπλυση του τραύματος και συρραφή κατά στρώματα. Τέλος, τοποθετήθηκε μηροκνημικός νάρθηκας σε έκταση.`,

        "Διατροχαντήριο κάταγμα": `Με τον ασθενή υπό αναισθησία και στο τραπέζι των ηλώσεων, πραγματοποιήθηκε ανάταξη του κατάγματος υπό ακτινοσκοπικό έλεγχο. Έξω πλάγια προσπέλαση και διάνοιξη της πλατείας περιτονίας. Εντοπισμός του σημείου εισόδου στον μείζονα τροχαντήρα. Τοποθέτηση οδηγού εντός του αυλού του μηριαίου και γλυφανισμός. Τοποθέτηση ενδομυελικού ήλου, διαυχενικού κοχλία και περιφερικής βίδας ασφάλισης. Ακτινοσκοπικός έλεγχος της ανάταξης και της θέσης των υλικών. Έλεγχος αιμόστασης, πλύσεις και συρραφή κατά στρώματα.`,

        "Ολική αρθροπλαστική γόνατος": `Με τον ασθενή υπό αναισθησία, πραγματοποιήθηκε αντισηψία και αποστείρωση του χειρουργικού πεδίου. Πρόσθια μέση τομή γόνατος και προσπέλαση της άρθρωσης. Εκτελέστηκαν οι προβλεπόμενες οστεοτομίες και τοποθετήθηκαν τα δοκιμαστικά υλικά. Μετά τον έλεγχο της σταθερότητας, της ευθυγράμμισης και του εύρους κίνησης, τοποθετήθηκαν τα οριστικά εμφυτεύματα. Έλεγχος αιμόστασης, πλύσεις και συρραφή κατά στρώματα.`,

        "Ολική αρθροπλαστική ισχίου": `Με τον ασθενή υπό αναισθησία, πραγματοποιήθηκε αντισηψία και αποστείρωση του χειρουργικού πεδίου. Προσπέλαση της άρθρωσης του ισχίου και παρασκευή κατά στρώματα. Εκτομή της μηριαίας κεφαλής, προετοιμασία της κοτύλης και του μηριαίου αυλού. Τοποθέτηση των δοκιμαστικών και στη συνέχεια των οριστικών εμφυτευμάτων. Έλεγχος σταθερότητας, μήκους και εύρους κίνησης. Αιμόσταση, πλύσεις και συρραφή κατά στρώματα.`,

        "Κάταγμα άπω άκρου κερκίδας": `Με τον ασθενή υπό αναισθησία, πραγματοποιήθηκε αντισηψία και αποστείρωση του χειρουργικού πεδίου. Προσπέλαση του άπω άκρου της κερκίδας. Ανάταξη του κατάγματος υπό ακτινοσκοπικό έλεγχο και προσωρινή σταθεροποίηση. Τοποθέτηση πλάκας και κοχλιών. Ακτινοσκοπικός έλεγχος της ανάταξης, του μήκους των κοχλιών και της θέσης των υλικών. Αιμόσταση, πλύσεις και συρραφή κατά στρώματα.`
    };

    A.modules.medicalCardData = {
        loaded: true,
        version: '0.15.0'
    };

    console.log('Asklipios medical card data loaded', {
        diagnosisPresets:
            Object.keys(A.data.DIAGNOSIS_OPTIONS || {}).length,
        medicalCardPresets:
            Object.keys(A.data.MEDICAL_CARD_PRESETS || {}).length,
        medicalActPresets:
            Object.keys(A.data.MEDICAL_ACT_OPTIONS || {}).length,
        companies:
            (A.data.MEDICAL_CARD_COMPANIES || []).length,
        surgeryDescriptions:
            Object.keys(A.data.SURGERY_DESCRIPTIONS || {}).length
    });
})();
